...
sys.path.insert(0, str(Path(__file__).resolve().parents[2]))  # project root (ORCA/)
...
def experiment_tabledescription(data_dir: Path, result_dir: Path, timestamp: str, data: str, llm=None, db_id: str = None):
    ...
    for i, q in enumerate(questions):
        ...
        result = app.invoke({"input": q['table_name'], "db_id": db_id or q.get('db_id')})
...
def experiment_recommendtable(data_dir: Path, result_dir: Path, timestamp: str, data: str, llm=None, db_id: str = None):
    ...
    for i, q in enumerate(questions):
        ...
        result = app.invoke({"input": q['question'], "db_id": db_id or q.get('db_id')})
...
def experiment_text2sql_generator(data_dir: Path, result_dir: Path, timestamp: str, data: str, llm=None, db_id: str = None):
...
def main():
    ...
    parser.add_argument("-m", "--model", type=str, default="gpt-4o-mini",
                        help="LLM model name (e.g. gpt-4o-mini, gpt-5.2, gemini-2.5-flash)")
    parser.add_argument("-p", "--provider", type=str, default="openai",
                        choices=["openai", "google", "ollama"],
                        help="LLM provider")
    parser.add_argument("--db-id", type=str, default=None,
                        help="Override db_id for all questions (e.g. reef_db). "
                             "Falls back to the db_id field in each question file.")
    ...
    data_dir = Path(f"experiments_v0/questions/{data}")
    result_dir = Path(f"experiments/results/{data}/{model_tag}")
    os.makedirs(result_dir, exist_ok=True)

    print(f"Model: {args.model}  Provider: {args.provider}  Dataset: {data}  DB: {db_id or '(from question file)'}")
    print(f"Results → {result_dir}\n")

    if "table_explorer" in tasks:
        experiment_tabledescription(data_dir, result_dir, timestamp, data, llm=llm, db_id=db_id)
    if "table_recommender" in tasks:
        experiment_recommendtable(data_dir, result_dir, timestamp, data, llm=llm, db_id=db_id)
    if "text2sql_generator" in tasks:
        experiment_text2sql_generator(data_dir, result_dir, timestamp, data, llm=llm, db_id=db_id)
    # if "causal_analysis" in tasks:
    #     experiment_causalanalysis(data_dir, result_dir, timestamp, data, llm=llm, db_id=db_id)
    if "main_agent" in tasks:
        experiment_mainagent(data_dir, result_dir, timestamp, data, llm=llm)
